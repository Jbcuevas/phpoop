...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['search'])) {
        ...
        try {
            ...
            if ($connection) {
                // SQL query with JOIN
                $query = $connection->prepare("SELECT users.user_id, users.user_firstname, users.user_lastname, users.user_birthday, users.user_sex, users.user_email,  users.user_name, users.user_profile_picture, 
                 CONCAT(user_address.city,', ', user_address.province) AS address FROM users INNER JOIN user_address ON users.user_id = user_address.user_id WHERE users.user_name LIKE ? OR users.user_email LIKE ? OR users.user_id LIKE ? OR CONCAT(user_address.city,', ', user_address.province) LIKE ? ");
